...
void CounterMap::initCounterMap(
  const Vec3i & half_prob_map_size_i, /* The input is half map size, to ensure
                                          the map size is always odds*/
  const double & prob_map_resolution, const double & temp_counter_map_resolution,
  const int & inflation_step, const bool & map_sliding_en, const double & sliding_thresh,
  const Vec3f & fix_map_origin, const double & unk_thresh)
{
  ...
  /* 1) compute the resolution ratio, ensure the resolution is an integer (for
   * CORNER) or odd numbered multiple (for CENTER)
   * */
  if (prob_map_resolution > temp_counter_map_resolution) {
    throw std::invalid_argument(
      " -- [CounterMap]: prob_map_resolution should be smaller than or equal "
      "to counter_map_resolution!");
  }
  ...
  std::cout << YELLOW << " -- [CounterMap] inflation_ratio: " << inflation_ratio << '\n';
#endif
  /* 2) compute the counter sliding map size i, which should be larger than prob
   * map and consider the inflation step, to ensure the counter is correctly
   * updated
   * */
  ...
  initSlidingMap(
    half_counter_map_size_i, counter_map_resolution, map_sliding_en, sliding_thresh,
    fix_map_origin);
  ...
  resetLocalMap();
  std::cout << GREEN << " -- [CounterMap] Init successfully -- ." << RESET << '\n';
  printMapInformation();
}

void CounterMap::updateGridCounter(
  const Vec3f & pos, const GridType & from_type, const GridType & to_type)
{
  /*
   ...
   * when unk_cnt > sub_grid_num * unk_ratio, this grid is considered to be
   * unknown?
   * */
  ...
  if (md_.occupied_cnt[addr] < 0 || md_.occupied_cnt[addr] > md_.sub_grid_num) {
    std::cout << YELLOW << "From type: " << GridTypeStr[from_type] << RESET << '\n';
    std::cout << YELLOW << "To type: " << GridTypeStr[to_type] << RESET << '\n';
    std::cout << YELLOW << "Occupied counter: " << md_.occupied_cnt[addr] << RESET << '\n';
  ...
  if (md_.unknown_cnt[addr] < 0 || md_.unknown_cnt[addr] > md_.sub_grid_num) {
    std::cout << YELLOW << "From type: " << from_type << RESET << '\n';
    std::cout << YELLOW << "To type: " << to_type << RESET << '\n';
    std::cout << YELLOW << "Unknown counter: " << md_.unknown_cnt[addr] << RESET << '\n';
