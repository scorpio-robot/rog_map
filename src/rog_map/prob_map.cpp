...
void ProbMap::initProbMap()
{
  ...
  init_once = true;
  initSlidingMap(
    cfg_.half_map_size_i, cfg_.resolution, cfg_.map_sliding_en, cfg_.map_sliding_thresh,
    cfg_.fix_map_origin);
  ...
  if (cfg_.frontier_extraction_en) {
    fcnt_map_ = std::make_shared<FreeCntMap>(
      cfg_.half_map_size_i + Vec3i::Constant(2), cfg_.resolution, cfg_.map_sliding_en,
      cfg_.map_sliding_thresh, cfg_.fix_map_origin);
  }

  if (cfg_.esdf_en) {
    esdf_map_ = std::make_shared<ESDFMap>();
    esdf_map_->initESDFMap(
      cfg_.half_map_size_i, cfg_.resolution, cfg_.esdf_resolution, cfg_.esdf_local_update_box,
      cfg_.map_sliding_en, cfg_.map_sliding_thresh, cfg_.fix_map_origin, cfg_.unk_thresh);
  }
  ...
  cout << "[ProbMap] virtual_ceil_height: " << cfg_.virtual_ceil_height << '\n';
  cout << "[ProbMap] virtual_ground_height: " << cfg_.virtual_ground_height << '\n';

  if (!cfg_.map_sliding_en) {
    std::cout << YELLOW << " -- [ProbMap] Map sliding disabled, set origin to ["
              << cfg_.fix_map_origin.transpose() << "] -- " << RESET << '\n';
  ...
  std::cout << GREEN << " -- [ProbMap] Init successfully -- ." << RESET << '\n';
  printMapInformation();
}
...
{
  log_file << "[ProbMap]" << '\n';
  log_file << "\tmap_size_d: " << cfg_.map_size_d.transpose() << '\n';
  log_file << "\tresolution: " << cfg_.resolution << '\n';
  log_file << "\tmap_size_i: " << sc_.map_size_i.transpose() << '\n';
  log_file << "\tlocal_update_box_size: " << cfg_.local_update_box_d.transpose() << '\n';
  log_file << "\tp_min: " << cfg_.p_min << '\n';
  log_file << "\tp_max: " << cfg_.p_max << '\n';
  log_file << "\tp_hit: " << cfg_.p_hit << '\n';
  log_file << "\tp_miss: " << cfg_.p_miss << '\n';
  log_file << "\tp_occ: " << cfg_.p_occ << '\n';
  log_file << "\tp_free: " << cfg_.p_free << '\n';
  log_file << "\tunk_thresh: " << cfg_.unk_thresh << '\n';
  log_file << "\tmap_sliding_thresh: " << cfg_.map_sliding_thresh << '\n';
  log_file << "\tmap_sliding_en: " << cfg_.map_sliding_en << '\n';
  log_file << "\tfix_map_origin: " << cfg_.fix_map_origin.transpose() << '\n';
  log_file << "\tvisualization_range: " << cfg_.visualization_range.transpose() << '\n';
  log_file << "\tvirtual_ceil_height: " << cfg_.virtual_ceil_height << '\n';
  log_file << "\tvirtual_ground_height: " << cfg_.virtual_ground_height << '\n';
  log_file << "\tbatch_update_size: " << cfg_.batch_update_size << '\n';
  log_file << "\tfrontier_extraction_en: " << cfg_.frontier_extraction_en << '\n';
...
void ProbMap::updateProbMap(const PointCloud & cloud, const Pose & pose)
{
  ...
  if (
    raycast_data_.batch_update_counter == 0 && cfg_.map_sliding_en &&
...
{
  // NOTE, we consider, if the pos is not inside prob map, it is also out of inf
  // map.
  if (!insideLocalMap(pos)) {
    return OUT_OF_MAP;
  }
...
void ProbMap::boxSearch(
  const Vec3f & _box_min, const Vec3f & _box_max, const GridType & gt,
  vec_E<Vec3f> & out_points) const
{
  out_points.clear();
  if (map_empty_) {
    std::cout << YELLOW << " -- [ROG] Map is empty, cannot perform box search." << RESET << '\n';
    return;
  }
  if ((_box_max - _box_min).minCoeff() <= 0) {
    std::cout << YELLOW << " -- [ROG] Box search failed, box size is zero." << RESET << '\n';
    return;
  }
...
void ProbMap::boxSearchInflate(
  const Vec3f & box_min, const Vec3f & box_max, const GridType & gt,
  vec_E<Vec3f> & out_points) const
{
...
void ProbMap::boundBoxByLocalMap(Vec3f & box_min, Vec3f & box_max) const
{
  if ((box_max - box_min).minCoeff() <= 0) {
    box_min = box_max;
    std::cout << YELLOW << "-- [ROG] Bound box is invalid." << RESET << '\n';
    return;
  }
...
void ProbMap::probabilisticMapFromCache()
{
  ...
  while (!raycast_data_.update_cache_id_g.empty()) {
    ...
    if (raycast_data_.hit_cnt[hash_id] > 0) {
      ...
      missPointUpdate(
        pos, hash_id, raycast_data_.operation_cnt[hash_id] - raycast_data_.hit_cnt[hash_id]);
    }
...
void ProbMap::updateLocalBox(const Vec3f & cur_odom)
{
   ...
   * the map origin and map bound is only update in
   * [SlidingMap::mapSliding(const Vec3f &odom)]
   * */
  ...
  raycast_data_.local_update_box_max =
    raycast_data_.local_update_box_max.cwiseMin(local_map_bound_max_d_);
  raycast_data_.local_update_box_min =
    raycast_data_.local_update_box_min.cwiseMax(local_map_bound_min_d_);
}

void ProbMap::resetLocalMap()
{
  std::cout << YELLOW << " -- [Prob-Map] Clear all local map." << RESET << '\n';
