...
bool RayCaster::setInput(const Eigen::Vector3d & start, const Eigen::Vector3d & end)
{
  ...
  //                std::cout<<"expand_dir_x_:"<<expand_dir_x_<<"
  //                expand_dir_y_:"<<expand_dir_y_<<"
  //                expand_dir_z_:"<<expand_dir_z_<<'\n';
  ...
  t_max =
    sqrt(dis_x_over_t * dis_x_over_t + dis_y_over_t * dis_y_over_t + dis_z_over_t * dis_z_over_t);
  ...
  t_when_step_x_ =
    expand_dir_x_ == 0 ? std::numeric_limits<double>::max() : std::abs(resolution_ / dis_x_over_t);
  t_when_step_y_ =
    expand_dir_y_ == 0 ? std::numeric_limits<double>::max() : std::abs(resolution_ / dis_y_over_t);
  t_when_step_z_ =
    expand_dir_z_ == 0 ? std::numeric_limits<double>::max() : std::abs(resolution_ / dis_z_over_t);
  ...
  //                std::cout << "t_to_bound_x_: " << t_to_bound_x_ << "
  //                t_to_bound_y_: " << t_to_bound_y_
  //                          << " t_to_bound_z_: " << t_to_bound_z_ <<
  //                          '\n';
...
bool RayCaster::step(Eigen::Vector3d & ray_pt)
{
...
#ifdef ORIGIN_AT_CENTER
  throw std::runtime_error(
    " -- [RayCaster]: ORIGIN_AT_CORNER and "
    "ORIGIN_AT_CENTER cannot be both true!");
#endif
#endif
...
#endif
  step_num_++;
  //                std::cout << "Iter " << step_num_ <<
  //                "========================================" << '\n';
  //                std::cout << "start: " << start_x_d_ << ", " << start_y_d_
  //                << ", " << start_z_d_ << '\n'; std::cout << "end: " <<
  //                end_x_d_ << ", " << end_y_d_ << ", " << end_z_d_ <<
  //                '\n'; std::cout << "end_id: " << end_x_i_ << ", " <<
  //                end_y_i_ << ", " << end_z_i_ << '\n'; std::cout <<
  //                "cur_id: " << cur_ray_pt_id_x_ << ", " << cur_ray_pt_id_y_
  //                << ", " << cur_ray_pt_id_z_
  //                          << '\n';
  //                std::cout << "t_to_bound
  //                : " << t_to_bound_x_ << ", " << t_to_bound_y_ << ", " <<
  //                t_to_bound_z_
  //                          << '\n';
  // shape the output
  ...
  if (
    cur_ray_pt_id_x_ == end_x_i_ && cur_ray_pt_id_y_ == end_y_i_ && cur_ray_pt_id_z_ == end_z_i_) {
    return false;
  }

  // compute first bound point
