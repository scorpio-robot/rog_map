...
void ESDFMap::initESDFMap(
  const rog_map::Vec3i & half_prob_map_size_i, const double & prob_map_resolution,
  const double & temp_counter_map_resolution, const rog_map::Vec3f & local_update_box,
  const bool & map_sliding_en, const double & sliding_thresh, const rog_map::Vec3f & fix_map_origin,
  const double & unk_thresh)
{
  ...
  initCounterMap(
    half_prob_map_size_i, prob_map_resolution, temp_counter_map_resolution, 0, map_sliding_en,
  ...
  resetLocalMap();
  std::cout << GREEN << " -- [ESDFMap] Init successfully -- ." << RESET << '\n';
  printMapInformation();
}
...
{
  std::cout << YELLOW << " -- [ESDFMap] Clear all local map." << RESET << '\n';
...
void ESDFMap::updateESDF3D(const rog_map::Vec3f & cur_odom)
{
      ...
      fillESDF(
        [&](int y) {
          return tmp_buffer1_[getEsdfLocalIndexHash(
            x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0], y,
      ...
      fillESDF(
        [&](int x) {
          return tmp_buffer2_[getEsdfLocalIndexHash(
            x, y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
            z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])];
        },
        [&](int x, double val) {
  ...
  t += up_t.stop();
  cnt++;
  std::cout << "Average time per point: " << t / cnt / sc_.map_vox_num * 1e6 << "us" << '\n';
#endif
}

void ESDFMap::resetOneCell(const int & hash_id) {}

void ESDFMap::getPositiveESDFPointCloud(
  const rog_map::Vec3f & box_min_d, const rog_map::Vec3f & box_max_d, const double & visualize_z,
...
void ESDFMap::getNegativeESDFPointCloud(
  const rog_map::Vec3f & box_min_d, const rog_map::Vec3f & box_max_d, const double & visualize_z,
  pcl::PointCloud<pcl::PointXYZI> & pcl_pc)
{
  std::lock_guard<std::mutex> lck(update_esdf_mtx);
...
void ESDFMap::fillESDF(
  F_get_val f_get_val, F_set_val f_set_val, const int & start, const int & end, const int & dim,
  const int & id_l)
{
  if (end < 0) return;
...
void ESDFMap::interpolateTrilinearEDT(
