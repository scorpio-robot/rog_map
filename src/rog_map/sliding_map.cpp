...
SlidingMap::SlidingMap(
  const Vec3i & half_map_size_i, const double & resolution, const bool & map_sliding_en,
  const double & sliding_thresh, const Vec3f & fix_map_origin)
{
  std::cout << "half_map_size_i: " << half_map_size_i.transpose() << '\n';
  std::cout << "resolution: " << resolution << '\n';
  std::cout << "map_sliding_en: " << map_sliding_en << '\n';
  std::cout << "sliding_thresh: " << sliding_thresh << '\n';
  std::cout << "fix_map_origin: " << fix_map_origin.transpose() << '\n';
  initSlidingMap(half_map_size_i, resolution, map_sliding_en, sliding_thresh, fix_map_origin);
}

void SlidingMap::initSlidingMap(
  const rog_map::Vec3i & half_map_size_i, const double & resolution, const bool & map_sliding_en,
...
{
  std::cout << GREEN << "\tresolution: " << sc_.resolution << RESET << '\n';
  std::cout << GREEN << "\tmap_sliding_en: " << sc_.map_sliding_en << RESET << '\n';
  std::cout << GREEN << "\tlocal_map_size_i: " << sc_.map_size_i.transpose() << RESET << '\n';
  std::cout << GREEN
            << "\tlocal_map_size_d: " << sc_.map_size_i.cast<double>().transpose() * sc_.resolution
            << RESET << '\n';
}
...
void SlidingMap::updateLocalMapOriginAndBound(
  const rog_map::Vec3f & new_origin_d, const rog_map::Vec3i & new_origin_i)
{
...
void SlidingMap::globalIndexToLocalIndex(const Vec3i & id_g, Vec3i & id_l) const
{
  for (int i = 0; i < 3; ++i) {
    ...
    //        id_l(i) += id_l(i) > sc_.half_map_size_i(i) ? -sc_.map_size_i(i) :
    //        0; id_l(i) += id_l(i) < -sc_.half_map_size_i(i) ?
    //        sc_.map_size_i(i) : 0;
  }
}
