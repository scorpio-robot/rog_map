...
bool InfMap::isUnknownInflate(const Vec3f & pos) const
{
  if (!cfg_.unk_inflation_en) {
    throw std::runtime_error(
      "Unknown inflation is not enabled, but the isUnknownInflate API is "
      "called, which should not happen.");
  }
...
InfMap::InfMap(rog_map::Config & cfg)
{
  ...
  initCounterMap(
    cfg.half_map_size_i, cfg.resolution, cfg.inflation_resolution, max_step, cfg.map_sliding_en,
  ...
  resetLocalMap();
  cfg_ = cfg;
  std::cout << GREEN << " -- [InfMap] Init successfully -- ." << RESET << '\n';
  printMapInformation();
}
...
{
  log_file << "[InfMap]" << '\n';
  log_file << "\tresolution: " << sc_.resolution << '\n';
  log_file << "\tmap_size_i: " << sc_.map_size_i.transpose() << '\n';
  log_file << "\tmap_size_d: " << (sc_.map_size_i.cast<double>() * sc_.resolution).transpose()
           << '\n';
}
  ...
  vec_E<Vec3f> & out_points) const
{
  ...
  if (gt == UNKNOWN) {
    if (!cfg_.unk_inflation_en) {
      out_points.clear();
      std::cout << YELLOW
                << " -- [ROG] Unknown inflation is not enabled, cannot perform "
                   "box search."
                << RESET << '\n';
      return;
    }
...
{
  std::cout << YELLOW << " -- [Inf-Map] Clear all local map." << RESET << '\n';
...
void InfMap::updateUnkInflation(const Vec3i & id_g, const bool is_add)
{
  TimeConsuming tc("updateInflation", false);
  if (!cfg_.unk_inflation_en) {
    std::cout << YELLOW << "Cannot updateUnkInflation of InfMap when unk_inflation_en is false."
              << RESET << '\n';
    return;
  }
  ...
  for (const auto & nei : cfg_.unk_inf_spherical_neighbor) {
    ...
    if (imd_.unk_inflate_cnt[addr] < 0 || imd_.unk_inflate_cnt[addr] > imd_.unk_neighbor_num) {
      std::cout << "unk_inflate_cnt: " << imd_.unk_inflate_cnt[addr]
                << " unk_neighbor_num: " << imd_.unk_neighbor_num << '\n';
      throw std::runtime_error(" -- [IM] Negative occupancy counter, which should not happened.!");
    }
#endif
  }
...
void InfMap::triggerJumpingEdge(
  const rog_map::Vec3i & id_g, const rog_map::GridType & from_type,
