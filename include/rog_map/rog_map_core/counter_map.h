...
protected:
  ...
  GridType getGridType(const int & hash_id) const
  {
     ...
     *  2.1) if the number of unknown sub-cell is larger than unk_thresh, the
     * grid is unknown 2.2) else this grid is known free
     * */
  ...
  virtual void triggerJumpingEdge(
    const Vec3i & id_g, const GridType & from_type, const GridType & to_type) = 0;
