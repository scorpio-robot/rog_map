...
#error \
  "Cannot use both ORIGIN_AT_CENTER and ORIGIN_AT_CENTER at the same time. Please define only one."
#endif
#endif
...
public:
  ...
  Config(const string & cfg_path, const string & name_space = "rog_map")
  {
    ...
    throw std::runtime_error(
      " -- [SlidingMap]: ORIGIN_AT_CORNER and "
      "ORIGIN_AT_CENTER cannot be both true!");
#endif
#endif
    ...
    throw std::runtime_error(
      " -- [SlidingMap]: ORIGIN_AT_CORNER and "
      "ORIGIN_AT_CENTER cannot be both false!");
#endif
#endif
    ...
    loader.LoadParam(
      name_space + "/esdf/local_update_box", temp_esdf_update_box, temp_esdf_update_box);
    ...
    loader.LoadParam(
      name_space + "/ros_callback/cloud_topic", cloud_topic, string("/cloud_registered"));
    loader.LoadParam(
      name_space + "/ros_callback/odom_topic", odom_topic, string("/lidar_slam/odom"));
    ...
    loader.LoadParam(
      name_space + "/visualization/use_dynamic_reconfigure", use_dynamic_reconfigure, false);
    ...
    if (temp_vis_range.size() != 3) {
      ...
      if (visualization_range.minCoeff() <= 0) {
        std::cout << color_text::YELLOW
                  << " -- [ROG] Visualization range is not set, visualization "
                     "is disabled"
                  << color_text::RESET << '\n';
    ...
    if (point_filt_num <= 0) {
      std::cout << color_text::YELLOW
                << " -- [ROG] point_filt_num should be larger or equal than 1, "
                   "it is set to 1 now."
                << RESET << '\n';
      point_filt_num = 1;
    }
    ...
    if (batch_update_size <= 0) {
      std::cout << color_text::YELLOW
                << " -- [ROG] batch_update_size should be larger or equal than "
                   "1, it is set to 1 now."
                << RESET << '\n';
      batch_update_size = 1;
    }
...
#define logit(x) (log((x) / (1 - (x))))
    ...
    std::cout << BLUE << "\t[ROG] n_free: " << n_free << RESET << '\n';
    std::cout << BLUE << "\t[ROG] n_occ: " << n_occ << RESET << '\n';

    std::cout << BLUE << "\t[ROG] l_hit: " << l_hit << RESET << '\n';
    std::cout << BLUE << "\t[ROG] l_miss: " << l_miss << RESET << '\n';
    std::cout << BLUE << "\t[ROG] l_min: " << l_min << RESET << '\n';
    std::cout << BLUE << "\t[ROG] l_max: " << l_max << RESET << '\n';
    std::cout << BLUE << "\t[ROG] l_occ: " << l_occ << RESET << '\n';
    std::cout << BLUE << "\t[ROG] l_free: " << l_free << RESET << '\n';
    ...
    for (int dx = -inflation_step; dx <= inflation_step; dx++) {
      for (int dy = -inflation_step; dy <= inflation_step; dy++) {
        for (int dz = -inflation_step; dz <= inflation_step; dz++) {
          if (
            inflation_step == 1 || dx * dx + dy * dy + dz * dz <= inflation_step * inflation_step) {
    ...
    std::sort(
      inf_spherical_neighbor.begin(), inf_spherical_neighbor.end(),
    ...
    if (unk_inflation_en) {
      ...
      std::sort(
        unk_inf_spherical_neighbor.begin(), unk_inf_spherical_neighbor.end(),
        [](const Vec3i & a, const Vec3i & b) {
          return a.x() * a.x() + a.y() * a.y() + a.z() * a.z() <
                 b.x() * b.x() + b.y() * b.y() + b.z() * b.z();
        });
    ...
    std::sort(
      spherical_neighbor.begin(), spherical_neighbor.end(), [](const Vec3i & a, const Vec3i & b) {
        return a.x() * a.x() + a.y() * a.y() + a.z() * a.z() <
               b.x() * b.x() + b.y() * b.y() + b.z() * b.z();
      });
  }
   ...
   *  the float type ceil and ground height is remap to the lower and upper
   * bound of the inflation layer respectively
   */
  double virtual_ceil_height{}, virtual_ground_height{};
  int inf_virtual_ceil_height_id_g{}, inf_virtual_ground_height_id_g{};

  bool visualization_en{false}, frontier_extraction_en{false}, raycasting_en{true},
    ros_callback_en{false}, pub_unknown_map_en{false};
  ...
  void resetMapSize()
  {
    ...
    std::cout << YELLOW << " -- [RM-Config] inflation_ratio: " << inflation_ratio << '\n';
#endif

    inflation_resolution = resolution * inflation_ratio;
    std::cout << color_text::GREEN
              << " -- [RM-Config] inflation_resolution: " << inflation_resolution
              << color_text::RESET << '\n';
    ...
    // 2) we calculate the index number of the prob map, which should be smaller
    // than infmap
...
#ifdef ORIGIN_AT_CENTER
    inf_virtual_ceil_height_id_g = static_cast<int>(
      virtual_ceil_height / inflation_resolution + SIGN(virtual_ceil_height) * 0.5);
    inf_virtual_ground_height_id_g = static_cast<int>(
      virtual_ground_height / inflation_resolution + SIGN(virtual_ground_height) * 0.5);
    ...
    virtual_ceil_height =
      inf_virtual_ceil_height_id_g * inflation_resolution - 0.5 * inflation_resolution;
    virtual_ground_height =
      inf_virtual_ground_height_id_g * inflation_resolution + 0.5 * inflation_resolution;
    fmt::print(
      " -- [ROG-Map] Init, resetMapSize: virtual_ceil_height: {}, "
      "virtual_ground_height: {}.\n",
