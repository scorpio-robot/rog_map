...
public:
  ...
  {
    path_ = file_path;
    std::cout << "Load config file: " << path_ << '\n';
  }

  template <typename T>
  bool LoadParam(
    const std::string & param_name, T & param_value, const T & default_value = T{},
  ...
  template <class T>
  bool LoadParam(
    const std::string & param_name, std::vector<T> & param_value,
      ...
      if constexpr (is_vector<T>::value) {
        if (tmp_node.Type() == YAML::NodeType::Sequence) {
          try {
            ...
            printf(
              "\033[0;31m Type mismatch for param %s, using default value: \033[0;0m",
              param_name.c_str());
